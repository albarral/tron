...
ArmClient::ArmClient()
{    
    pArmJointClient = new ArmChannelClient(ArmTopics::eARM_JOINT);
    pArmAxisClient = new ArmChannelClient(ArmTopics::eARM_AXIS);
    pArmCyclicClient = new ArmChannelClient(ArmTopics::eARM_CYCLIC);
    pArmExtraClient = new ArmChannelClient(ArmTopics::eARM_EXTRA);
}

ArmClient::~ArmClient()
{    
    delete(pArmJointClient);
    delete(pArmAxisClient);
    delete(pArmCyclicClient);
    delete(pArmExtraClient);
}
...
bool ArmClient::setFrontCyclerAmp1(float value)
{
    ...
    return pArmCyclicClient->sendMessage(CyclicTalker::eCYCLIC_FRONT1_AMP, value);    
}
...
bool ArmClient::stopArm()
{
    ...
    return pArmExtraClient->sendMessage(BasicTalker::eEXTRA_ARM_STOP, 0.0);        
}

bool ArmClient::keepTilt()
{
    LOG4CXX_DEBUG(logger, "ArmClient: keep tilt > ");     
    return pArmExtraClient->sendMessage(BasicTalker::eEXTRA_KEEP_TILT, 0.0);        
}

bool ArmClient::endArm()
{
    LOG4CXX_DEBUG(logger, "ArmClient: end arm > ");     
    return pArmExtraClient->sendMessage(BasicTalker::eEXTRA_ARM_END, 0.0);        
}
}